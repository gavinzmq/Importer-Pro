...
R01–R14 **全部纳入实现计划**，无暂缓项；优先级仅表示交付版本：

| 优先级 | 含义 | 目标版本 |
| :--- | :--- | :--- |
| **P0** | 发布必备，基于既有蓝图能力补细节/UI | v1.0.0（2026-11-01，M6） |
| **P1** | 进阶能力，需新增模块设计 | v1.1（M7，2026-12-15 暂定） |
| **P2** | 增强能力，依赖 P1 或需专项设计 | v1.2（M8，2027-01-31 暂定） |
...
| R02 | HTML 网页导入（正文提取） | 官方 Defuddle | P2 | v1.2（M8） | 已排期 | 单文件/文件夹/ZIP；去除导航、侧边栏等干扰 |
| R03 | 字段类型智能推断 | obsidian-data-importer | P1 | v1.1 | 待开始 | 自动分析列类型并给出建议（如"90% 为日期"），用户可接受或覆盖 |
| R04 | 字段关系发现 / 唯一标识推荐 | obsidian-data-importer | P2 | v1.2（M8） | 已排期 | 识别一对一关系、推荐唯一标识、为智能链接提供关系建议（依赖 R03 类型推断） |
...
- **P1 项**（v1.1/M7）涉及新模块或接口扩展，开工前须在本组件补充对应设计并更新决策记录：
  ...
  - R07/R08：新增导入任务管理器（后台队列、进度通知、断点持久化于 `data.json` 的 `importer_id` 映射）
  - R03/R05：`TemplateScanner` 模板库 + 配置向导的类型推断/建议交互
  - R12/R13：Modal 拖拽入口、选择器搜索过滤
- **P2 项**（v1.2/M8）开工前同样须补充设计：
  - R02：基于 Defuddle 或等价正文提取引擎扩展 `HTMLParser`（单文件/文件夹/ZIP）
  - R04：复用 R03 的类型推断结果做字段关系启发式分析，输出唯一标识/外键建议
  - R14：模板变更历史与回滚（存储位置待定：模板目录历史文件或 `data.json`）
