...
> **表格类解析选项（D86/D123）**：`ExcelParser`/`CSVParser` 支持 `ParseOptions.rawRows`（原始行模式：全部物理行作为数据记录、占位列名 `列1..N`，供向导「表头 = 空行+行筛选+重复表头后剩余第一行（D124 顺序）」链路；缺省保持「第一行为表头」）；Excel 指定不存在的 `sheetName` 抛 `PARSE_002`（D86，不再静默返回空数组）；行清洗等预处理位于向导数据变换层（ui/layout.md §5.5）。
...
| **[💾 保存到模板] 按钮** | Step 3 区块 3 模板元信息操作行 [📝 编辑模板代码] [➕ 新建模板] [💾 保存到模板]（D94/D95；**D134 起增第四枚 [💾 保存到内容模板]**，已实现）——点击「保存到模板」即把 Step 3 全部配置编译并写回所选模板 preprocess 块；「保存到内容模板」= 把区块 5 行顺序/字段集写回正文 content 段（`saveContentTemplate`，仅写正文）；未选模板时禁用并提示先新建/选择；写入失败抛 `TEMPLATE_005` 内联提示；保存成功仅 Notice 不刷新页面 |
...
| **行清洗与表头（D122/D123/D124，D136 修订·已实现）** | **D136 起过滤空行/过滤重复表头编译为 Handlebars 段**——`row-clean`（row-filter 之前）：`{{#if (isEmptyRow this)}}{{set "_skip" true}}{{/if}}`；`row-header-dup`（row-filter 之后、column-mapping 之前）：`{{#if (isDuplicateHeader this _header)}}{{set "_skip" true}}{{/if}}`（`_header` = 引擎注入表头基准行快照，`isDuplicateHeader` = 新增公开 Helper）；引擎改**判定遍 + 渲染遍**（判定遍 = row-clean + row-filter 占位列名定 `_skip` → 基准行定位 → `promoteHeaderRow` 结构性提升（列名生成 + 基准行移除）→ 注入 `_header` → 渲染遍 = row-header-dup + 其余段提升后列名）；**表头 = 空行+行筛选后剩余第一行**；frontmatter `row.clean` 保留为开关（决定是否编译段）；API/非表格路径维持 `applyRowCleaning`（值==列名 + 空行一次完成，渲染前）；**原删除行 / 去重 / 过滤无效数据 / 合并行 / headerRow 已废弃删除** |
| **行筛选（D96；D136 修订·已实现）** | Excel 式包含式筛选：保留「全部规则（AND）均匹配」的行；**D136 起支持多组（组内 AND 保留、组间 OR 保留）**——`_skip` 特殊字段多行 = 多组规则组，与区块 4 行筛选多组展示一一对应双向同步，编译单组 `{{#unless (and …)}}{{set "_skip" true}}{{/unless}}`、多组 `{{#unless (or (and …组1) (and …组2))}}{{set "_skip" true}}{{/unless}}`；执行顺序在过滤空行之后、过滤重复表头之前（`空行 → 行筛选 → 表头定位/提升 → 重复表头 → 列映射`，向导表格类）；类型 `RowFilterRule` / `RowFilterOp` 见 §7；`RowFilterRule.column` 支持 `'*'` 任意列；旧 byContent 删除迁移为筛选规则（删除含 X = 筛选「任意列 不包含 X」，D97） |
...
| **区块 5 面板修订 + 行清洗 Handlebars 化（D136，2026-09-07 已实现）** | ① **设置下拉入面板**（修订 D135①/D117）——「设置」列仅 `⏵/⏷` + 恒显徽标，分组下拉移入「已添加设置」面板头部；② **特殊字段全入口可编辑双向同步**（修订 D133）——`_folder`/`_fileName` 面板行可编辑输出表达式、`_skip` 面板行可编辑规则组，与区块 3/4 同源共享、**取消权威编辑**（任一入口修改 → 广播 → 其余入口同步）；③ **`_skip`/`_link` 不唯一**——`filters` 为组数组（组内 AND、组间 OR，单组退化为现状形态）、`_link` 多行 = 多个 smartLink 候选（`push _link` 数组形态，`_link` 类型 `string | string[]`）；④ **特殊字段面板恒显 + 顶部「添加特殊字段」下拉**（修订 D135②）——入口从普通行「类型」下拉移除、改面板顶部新增（`_skip`/`_link` 恒可选），不再与普通行互移；⑤ **按钮行与可用源列置顶**——四枚按钮 + 「💡 可用源列」提示移至列映射表顶部；⑥ **行清洗 Handlebars 化**——新段 `row-clean`/`row-header-dup` + `_header` 快照 + `isDuplicateHeader` Helper、判定遍/渲染遍编排（详见「行清洗与表头」行）。决策见 decisions/2026-09-07-step3-block5-panel-revision-row-clean-handlebars.md（v1.1.0 implemented） |
...
[文件（按路径引用原文件）] → DataParser → DataRecord[]
    ...
    → DataPipeline → 预处理渲染（Handlebars 承载向导全部配置，均编译自 Step 3（D98）：行清洗 `row-clean`/`row-header-dup`（D136 已实现）/ 行筛选 `row-filter` / 列映射 `column-mapping`（含行内设置链，D113）/ 派生 `derived` / 输出位置 `output`（D129）/ 多笔记输出 `note-output`（D120）；表头提升为引擎结构性原语（core/row-clean.ts，D122/D123/D124/D136））→ 分流 → 派生字段
...
interface ParseOptions {
  ...
  rawRows?: boolean;   // 原始行模式（仅 Excel/CSV，D123）：全部物理行作为数据记录、占位列名 列1..N；供向导「表头=空行+筛选+重复表头后第一行（D124）」链路
}
 ...
 * （D130–D134，2026-09-06 已实现：`mappings` 数组序 = 内容模板顺序（段内 set 行序）、
 *   每行 `settings` 数组序 = 值管线/管道顺序（D131）；目标字段可取值含特殊字段
 *   `_skip`/`_folder`/`_fileName`/`_status`/`_warnings`/`_link`（唯一，D132/D133；
...
/** 行筛选规则：保留「全部规则均匹配」的行（多规则 AND）；执行顺序在过滤空行之后、过滤重复表头之前（D124，向导表格类 = 空行 → 行筛选 → 重复表头 → 表头提升）；D97：column 支持 '*' 任意列；D98：规则经编译层生成 preprocess Handlebars 条件块，不在运行时由 JS 执行 */
