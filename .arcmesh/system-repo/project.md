...
|**handlebars-helpers**|0.10.0|通用 Helper 实现源：白名单类别按名加载/委托（D102–D104 引入，2026-09-05 已实现；`src/helpers/handlebars-helpers.ts` 按名采纳 array/collection/comparison/math/number/string 六类重叠件，见 decisions/2026-09-05-handlebars-helpers-on-demand.md v1.2.0）|
...
| **测试** | 🟡 进行中（Vitest 单元已接入：`helpers`/`wizard-data`/`parsers`/`file-input`/`template-scanner` 纯函数共 114 例，含 D94–D98 行筛选/迁移/编译·反编译往返与真实渲染一致性用例、D99 pipe 管道语义/往返用例、D102 helpers 委托/改名/库语义对拍与编译例外专用名用例、模板配置读写往返；CI `ci:test` 消费；本地不跑门禁） |
| **文档** | 🟡 进行中 |
