...
*版本: 1.29.0 | 最后更新: 2026-09-06（D126–D129 已实现：条件校验 / 不输出 / itemAt 提取 / 输出位置编译段化；D130–D134 已实现：区块 5 行顺序 ↑/↓+拖拽（内容模板顺序，段内 moveMappingRow）/ 设置顺序 ↑/↓+拖拽（管道顺序，受限集）/ 特殊字段行（联动视图行 + 真实行 ipro:specialrow）/ 区块 3 保存到内容模板（applyContentLayout，仅主笔记），type-check 0 错、全量 Vitest 202 全绿，见 decisions/2026-09-06-step3-row-order-special-fields-content-template.md（v1.1.0 implemented））*