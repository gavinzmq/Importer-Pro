...
- **区块 5 交互增强 + 校验规则废弃（2026-09-06，D125 已实现）**：① **来源 → 目标自动清洗**——区块 5 来源下拉选择后目标字段自动更正为来源值去除全部空格/换行/回车后的值（`sourceToTargetName`，全空白源名回落原值；「添加映射行」自动填充、自动映射与依赖来源的派生缺省名同样清洗；来源变更 L1 预览就地更新不丢焦点）；② **输出到「所有笔记」**——映射行「输出到」下拉增「所有笔记」（noteType `'all'`：字段写入主笔记 + 全部附加笔记 object，编译展开、反编译按「主 + 全部附加类型对象均含」还原；无附加类型时等价主笔记零破坏）；③ **删除校验规则功能**——用户反馈「校验规则没用」：向导区块 4「✅ 校验规则」卡（D118）、预览 ✅/⚠️/❌ 标记、frontmatter `validation` 契约（旧模板读取忽略、保存不写出）与 D115 运行时接入（`ShardContext.validation`/`applyValidation`/`ImportRecordsOptions.validation` 移除；Validator 核心保留）全链路删除；保留字段 `_valid`/`_errors` 移除、`_warnings`（D119 附言）与 `_status`（模板可写）保留；公开校验 API（validate/validateField/getValidationRules/registerValidator/listValidators）标 @deprecated 保留一个 MINOR（v1.1 移除）；校验类 Helper 保留于公开清单。落点：wizard-data（sourceToTargetName/ALL_NOTE_TYPES 编译反编译）、import-modal、pipeline/import-service、template-scanner、types/api 注释、styles.css；单测：删 D118/D115 校验用例 9 例、新增 D125 清洗联动/输出到 all/契约废弃用例 7 例（全量 Vitest **169 例全绿**、type-check 通过）。蓝图同步：architecture 1.30.0 / ui/layout 1.24.0 / template-schema 1.17.0 / api-layer 1.8.0 / glossary 1.12.0 / project 1.31.0。见 decisions/2026-09-06-step3-mapping-ux-validation-removal.md（v1.1.0）
- **Step 3 配置增强（2026-09-06，D126–D129 已实现）**：① **条件校验（D126）**——区块 5「添加设置」增「条件校验」组：布尔 Helper 校验表达式（`validateID`/`isEmail`/`isPhone`/`isNumber`/`isDate`/`inRange`/`matchesRegex`/`isNotEmpty`/`isEmpty`）+ 参数 + 真值/假值（**固定值**字符串或**字段引用** `(lookup this "列名")`）；编译 = 整链替换式 `(ternary (校验fn 值 参数…) 真值 假值)`（同 D119 条件计算口径，单步直调、不入 pipe，反编译按校验函数/参数/分支形态还原）。② **输出到「不输出」（D127）**——映射行「输出到」增「不输出」（noteType `'none'`）：字段照常产 `set`（仅作预处理中间值，供设置链/行筛选/输出命名引用），但不进入任何笔记渲染数据（column-mapping 段写 `ipro:none:` 清单标记持久化、读取统一回填 noteType；`DataPipeline.shard` 按 `ctx.noneFields` 从主笔记与 `_notes` object 数据过滤；与「类型=忽略」不产 set 区别）。③ **数组/Object 提取（D128）**——「添加设置」增「提取」组：新公开 Helper `itemAt`（公开清单 37 → **38**，新类别「集合」）：数组 0-based 索引（负数自末尾）/ Object 键名取值，越界/缺键/非数组非对象返回 `''`；阶段白名单增 `itemAt`，编译 1 步直调 `(itemAt 源 索引|键)`、≥2 步 `(stage "itemAt" …)`（典型：拆分 → 取第 N 个）。④ **输出位置编译段化（D129）**——区块 3 输出位置/命名规则**不直接写进 frontmatter `output`**：编译进 preprocess 新段 `output`（derived 段之后、note-output 段之前：`{{#if (isNotEmpty (expr "…"))}}{{set "_folder" (expr "…")}}{{/if}}`，新增运行时 `expr` Helper 把完整模板文本按行渲染；folder/name 缺省不产段）；frontmatter `output.folder`/`note_name` **固定写 `"{{_folder}}"` / `"{{_fileName}}"`**（仅作引用保留字段的间接层）；D112 运行时求值保留为兜底；旧模板 frontmatter 非固定引用表达式读取/保存时一次性编译进 output 段并改写；`applyWizardTransform` 随阶段 B 渲染 output 段并注入确定性 `_hash` 占位。落点：builtin（`itemAt` + 白名单、`expr`）、wizard-data（validate/extract 设置模型、`NONE_NOTE_TYPE`/none 标记、`output` 段、编译/反编译往返）、template-scanner（frontmatter 固定引用 + output 段读写 + `config.noneFields`）、types（`TemplateConfig.noneFields`）、pipeline/import-service/api（shard `noneFields` 过滤）、import-modal（「添加设置」扩为 7 组下拉 + 条件校验/提取草稿表单、「输出到」增「不输出」、预览隐藏保留字段与 none）。单测：helpers +3、wizard-data +9、pipeline +2（全量 Vitest **183 例全绿**、type-check 通过）。蓝图同步：architecture 1.33.0 / ui/layout 1.26.0 / template-schema 1.19.0 / template-engine 1.10.0 / api-layer 1.10.0 / glossary 1.14.0 / CHANGELOG 1.27.0。决策见 decisions/2026-09-06-step3-mapping-output-enhancements.md（v1.1.0 implemented）
- **区块 5 顺序编排 + 特殊字段行 + 保存到内容模板（2026-09-06，D130–D134 已实现）**：① **行顺序调整（D130）**——区块 5 操作列 `↑/↓` + `⋮⋮` 拖拽重排映射行（仅同段内 `moveMappingRow`）：**行顺序 = 编译后段内 `set` 行序 = 内容模板（正文 content 段）字段呈现顺序**（D98 渲染顺序即代码顺序；只影响同段内顺序，跨段按段清单）；行序随 preprocess 代码顺序自然持久化、反编译按 set 行序回填；② **设置顺序调整（D131）**——行下「已添加设置」面板每项 `↑/↓`：**设置顺序 = 值管线执行顺序（管道顺序）**（1 步直调函数序 / ≥2 步 `pipe` 阶段序）；`类型` 隐含转换恒首步不参与重排、条件校验恒末步、warn/link 附言恒在 set 后（受限集）；③ **特殊字段行（D132）**——`_skip`/`_folder`/`_fileName`/`_status`/`_warnings`/`_link` 为保留字段可配置子集（`_index`/`_hash`/`_notes` 排除）：读模板按 `set "_xxx"` 识别为行、目标字段控件 = 输入 + 下拉（「特殊字段」分组，每字段**唯一**）、来源可空、类型与「输出到」禁用；④ **联动与专属设置（D133）**——`_folder`/`_fileName` ↔ 区块 3 输出位置/命名（D129 同源）、`_skip` ↔ 区块 4 行筛选（规则集共享）双向同步（上方区块为权威编辑入口）；「添加设置」对特殊字段行增**专属设置**（`_skip`=条件 / `_folder`·`_fileName`=输出表达式 / `_status`=固定值 / `_warnings`=条件警告 / `_link`=smartLink），新建特殊字段自动加入**默认设置**；⑤ **保存到内容模板（D134）**——区块 3 按钮行增 [💾 保存到内容模板]：把区块 5 行顺序与字段集写回模板正文 content 段（与 [💾 保存到模板] 独立、仅写正文；手写正文按 `{{字段}}` 引用行识别重排）。**实现（2026-09-06）**：落点 `wizard-data`（`moveMappingRow`/`moveRowSetting`/`SPECIAL_FIELDS`/`defaultSpecialSetting`/specialrow 编译·反编译/`mainNoteContentFields`）、`template-scanner`（`applyContentLayout`/`saveContentTemplate`）、`import-modal`（区块 5 ↑/↓+拖拽、目标字段特殊下拉、特殊字段视图行与联动、区块 3 第四枚按钮）、`styles.css`；type-check 0 错、全量 Vitest **202 例全绿**（wizard-data +11、template-scanner +8）。决策见 decisions/2026-09-06-step3-row-order-special-fields-content-template.md（v1.1.0 implemented）。实现后蓝图同步：architecture 1.35.0 / ui/layout 1.28.0 / template-schema 1.21.0 / glossary 1.16.0 / project 1.35.0
- **区块 5 列收敛与特殊字段面板（2026-09-06，D135 已实现）**：① **设置列**——「添加设置」列更名**「设置」**：分组下拉**留在列内**（不放入已添加设置面板），操作列 `⏵/⏷` 显隐按钮与数量徽标移入本列、**徽标恒显（0 也不隐藏）**；② **特殊字段入口（修订 D132）**——特殊字段下拉从目标字段移除，放入**「类型」下拉「特殊字段」分组**（选项 = **中文名**：跳过记录/目标文件夹/文件名/状态/警告列表/智能链接，每个唯一、已配置者灰置「已配置」）；「类型」下拉选中特殊字段后该行**从普通映射表挪移到「特殊字段面板」**（真实特殊字段行不参与普通行排序段，D130 口径不变）；③ **特殊字段面板**——独立面板行列 = **来源（可修改）/ 类型（不可修改、中文名）/ 目标字段（不可修改、含说明）/ 设置（可弹出已设置面板 + 徽标）/ 操作（删除）**：`_skip`/`_folder`/`_fileName` 为区块 4/3 **联动视图行**（来源显示 —、设置列显示联动摘要、✕ 清除对应上方区块配置并复位）；`_status`/`_warnings`/`_link` 为**真实行**（cfg.mappings + `ipro:specialrow` 反编译还原）；④ **设置下拉合并**——特殊字段行的「设置」下拉**同时含普通设置（七组）+ 特殊字段专属设置**（不再独立专属下拉）；特殊字段的**说明项（中文名 + 用途）**放入「已添加设置」面板；⑤ **排序**——特殊字段列表不需 `↑/↓` 排序按钮；⑥ **拖拽首列**——映射行拖拽把手 `⋮⋮` 移至表格**首列**、已添加设置拖拽把手放**设置项首列**；⑦ **笔记类型面板（修订 D120）**——生成条件下拉改为选择**列映射目标列（目标字段）**、`文件名后缀` 更名 **`文件名`**。落点：`wizard-data`（`SPECIAL_FIELD_LABELS` 增中文 `name`）、`import-modal`（renderMappingCard 列收敛 + 类型特殊分组、`renderSpecialFieldsPanel`/`renderSpecialViewRow`/`renderSpecialRealRow`/`renderSpecialRowAddSelect`、设置列重组、`renderMappingSettingsPanel` 说明项、`mappingTargetColumns`）、`styles.css`（首列把手/面板样式/设置列 flex）；单测 wizard-data +1（全量 Vitest **203 例全绿**、type-check 0 错）。决策见 decisions/2026-09-06-step3-block5-settings-special-fields-panel.md（v1.1.0 implemented）。蓝图同步：architecture 1.36.0 / ui/layout 1.29.0 / project 1.36.0
...
*版本: 1.30.0 | 最后更新: 2026-09-06（D126–D129 已实现：条件校验 / 不输出 / itemAt 提取 / 输出位置编译段化；D130–D134 已实现：区块 5 行顺序 ↑/↓+拖拽（内容模板顺序，段内 moveMappingRow）/ 设置顺序 ↑/↓+拖拽（管道顺序，受限集）/ 特殊字段行（联动视图行 + 真实行 ipro:specialrow）/ 区块 3 保存到内容模板（applyContentLayout，仅主笔记）；D135 已实现：区块 5 列收敛与特殊字段面板（设置列更名 + 徽标恒显、类型下拉特殊字段分组→挪移特殊字段面板、面板行列 来源/类型/目标字段/设置/操作、设置下拉合并、拖拽把手置首列、笔记类型生成条件目标列 + 文件名更名），type-check 0 错、全量 Vitest 203 全绿，见 decisions/2026-09-06-step3-block5-settings-special-fields-panel.md（v1.1.0 implemented））*