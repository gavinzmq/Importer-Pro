...
```json
{
  ...
  "scripts":{"dev":"node scripts/esbuild.config.mjs","ci:build":"tsc -noEmit -skipLibCheck && node scripts/esbuild.config.mjs production","type-check":"tsc -noEmit -skipLibCheck","ci:test":"vitest run --coverage --config scripts/vitest.config.ts","ci:lint":"eslint src --max-warnings 0","ci:package":"pnpm run ci:build && node scripts/package.mjs"},
...
- .gitignore: node_modules/ dist/ .env *.zip coverage/
- vitest.config: 位于 scripts/，environment node，alias obsidian→tests/stubs，coverage v8
